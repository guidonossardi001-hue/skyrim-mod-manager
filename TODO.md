...
- **Residuo (utente):** packaging reale dell'installer .exe non eseguito qui — serve `npm install` (bump a Electron 33 + rebuild ABI `better-sqlite3`) poi `npm run electron:build`. Vedi D1 sotto, ora aggiornato con l'icona/config definitive.
...
- **T13** *(confermato ancora aperto, 2026-07-10)* — `secrets/release_pub.pem` (113 byte) resta ridondante vs `docs/keys/release_pub.pem` (canonica). `secrets/release_priv.pem` risulta **già rimosso** dal filesystem (SEC-01 in parte eseguita) — verificare che la chiave privata sia stata effettivamente rilocata fuori dal repo con `SKYRIM_RELEASE_PRIV_KEY_PATH` prima di considerare SEC-01 chiuso al 100%.

## 🚢 Deployment
### ~~D1 — Produrre l'installer NSIS~~ → ✅ FATTO (2026-07-10)
- **Installer prodotto:** `release/Skyrim-AE-Fantasy-Launcher-Setup-1.0.0.exe` (89.7 MB, NSIS, electron 29.4.6, better-sqlite3 ricompilato ABI electron) + `latest.yml` (feed auto-update, url=file coerente, niente spazi) + `.blockmap` (delta update). `oneClick:false`, `perMachine:false` (no admin, dati preservati), shortcut Desktop+StartMenu "Skyrim AE Fantasy Launcher" con dragon icon, `runAfterFinish`, `deleteAppDataOnUninstall:false`. winCodeSign in cache → nessun problema symlink. **Non firmato** (`signAndEditExecutable:false`) → SmartScreen segnalerà l'app al primo avvio (normale per installer non firmati).
- **Residuo auto-update:** per attivarlo davvero, pubblicare una GitHub Release su `guidonossardi001-hue/skyrim-mod-manager` con tag `v1.0.0` allegando i 3 file (installer + `latest.yml` + `.blockmap`); i futuri bump di `version` in package.json + nuova release faranno aggiornare i client via `electron-updater`. Firma opzionale: `CSC_LINK`/`CSC_KEY_PASSWORD`.
