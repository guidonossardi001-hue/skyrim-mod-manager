...
>
> **RISOLTO 2026-07-16 (sera): build NSIS fresco ESEGUITO e VERIFICATO** su Electron 43.1.0 / electron-builder 26 con `disableAsarIntegrity:true` + `npmRebuild:false`: `release/Skyrim-AE-Fantasy-Launcher-Setup-1.0.1.exe` (132 MB) + `latest.yml` + `.blockmap` coerenti; win-unpacked rigenerata, app avviata e verificata viva (4 processi, provider Nexus attivo, log boot pulito). Nessun problema winCodeSign/symlink. (Build corrente: **1.0.2**.)
>
> **SCOPO CHIARITO 2026-07-17 — nessuna distribuzione.** L'utente ha stabilito che il launcher **non va dato a nessuno**: progetto a utente singolo, che è anche chi lo costruisce. Quindi **GitHub Release/auto-update e firma dell'installer NON servono** e non sono residui aperti — servono a chi distribuisce a terzi. Ogni `electron:build` riscrive `release/win-unpacked`, dove punta il collegamento Desktop: la versione nuova è già in mano all'utente. Senza feed nulla si rompe (`checkForLauncherUpdate` è best-effort: feed irraggiungibile → warning, avvio prosegue). Il codice per entrambe resta pronto: se lo scopo cambiasse, si accendono senza modifiche. Vedi il box "DECISIONE DI SCOPO" in TODO.md.
>
